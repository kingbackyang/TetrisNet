...
def load_checkpoint(model, optimizer, path):
    ...
    # state = {k: v for k, v in state.items() if 'decoder' not in k}
    # print(state)
    # state['model'].pop('decoder.word_convert.weight')
    # state['model'].pop('decoder.word_convert.bias')
    # state['model'].pop('decoder.embedding.weight')
    # state['model'].pop('counting_decoder1.pred_layer.0.weight')
    # state['model'].pop('counting_decoder2.pred_layer.0.weight')
    # state['model'].pop('decoder.counting_context_weight.weight')
    ...
    for key, value in model.state_dict().items():
        if key in  state["model"].keys():
            if value.size() == state["model"][key].size():
                ndict[key] = state["model"][key]

    model.load_state_dict(ndict, strict=True)
